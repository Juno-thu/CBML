...
class SessionGraph(Module):
    def __init__(self, opt: GlobalParam, n_node):
        ...
        self.mem_Wa = torch.nn.Parameter(
            torch.rand((self.hidden_size, self.memory_dim), dtype=torch.float32, requires_grad=True).cuda())
    ...
    def compute_scores1(self, item_seq_hidden, mask, self_att=True, residual=True, k_blocks=2):
...
def forward(model, i, data):
    ...
    return targets, model.compute_scores1(item_seq_hidden, mask)
